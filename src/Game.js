...
export default class Game {
	...
	getBet(player) {
		...
		return new Promise((resolve, reject) => {
			const checkBet = () => {
				let bet = 0;
				if (
					+this.betField.value > 0 &&
					/^[0-9]+$/.test(this.betField.value) &&
					!this.betField.value.startsWith("0")
				) {
			...
			const splitAction = async () => {
				if (player.money - hand.betSum >= 0) {
					...
					player.field.playingField.appendChild(
						newHand.playingField.htmlBlock
					);
					return true;
				} else {
	...
	countPoints() {
		...
		for (let player of this.players) {
			for (let hand of player.hands) {
				if (
					dealersHand.getHandPoints() <= 21 &&
					hand.getHandPoints() <= 21
				) {
					if (hand.getHandPoints() == dealersHand.getHandPoints()) {
						...
						player.money +=
							hand.getBetSum() + hand.getBetSum() * 1.5;
						return "Gewonnen";
					} else if (dealersHand.isBlackJack()) {
						hand.setResult("Du hast verloren");
						return "Verloren";
					} else if (
						dealersHand.getHandPoints() < hand.getHandPoints()
					) {
						hand.setResult("Du hast gewonnen");
