...
describe("hit()", () => {
	function checkHit() {
		...
		for (let i = 0; i < 10; i++) {
			hand.hit(deck.deck);
			if (
				hand.cards[currentCard] instanceof Card &&
				hand.cards[currentCard].value != null &&
				hand.cards[currentCard].suit != null &&
				hand.cards[currentCard].ui != null
			) {
				isCard = true;
			}
			currentCard++;
...
describe("split(player)", () => {
	function checkSplit(cards) {
		...
		it("check if length of both hands is 1", () => {
			expect(
				player.hands[0].cards.length && player.hands[1].cards.length
			).to.equal(1);
		});
